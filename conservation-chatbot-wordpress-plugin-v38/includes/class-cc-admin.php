...
class CC_Admin {
    ...
    public function init() {
        ...
        // Simple form handling for animal selection
    }
    ...
    public function sanitize_settings($input) {
        ...
        // Custom animals - process properly
        $sanitized['custom_animals'] = array();
        if (isset($input['custom_animals'])) {
            // Handle both JSON string and array formats
            $custom_animals_data = $input['custom_animals'];
            
            // If it's a JSON string, decode it
            if (is_string($custom_animals_data)) {
                $custom_animals_data = json_decode($custom_animals_data, true);
            }
            
            // If it's an array, process it
            if (is_array($custom_animals_data)) {
                foreach ($custom_animals_data as $animal_data) {
                    if (is_array($animal_data) && isset($animal_data['name']) && !empty($animal_data['name'])) {
                        $sanitized_animal = array(
                            'id' => sanitize_text_field($animal_data['id'] ?? 'custom_animal_' . uniqid()),
                            'name' => sanitize_text_field($animal_data['name']),
                            'type' => sanitize_text_field($animal_data['type'] ?? ''),
                            'story' => sanitize_textarea_field($animal_data['story'] ?? ''),
                            'image' => esc_url_raw($animal_data['image'] ?? ''),
                            'enabled' => isset($animal_data['enabled']) ? '1' : '0'
                        );
                        $sanitized['custom_animals'][] = $sanitized_animal;
                    }
                }
                // Limit to 10 custom animals to prevent memory issues
                $sanitized['custom_animals'] = array_slice($sanitized['custom_animals'], 0, 10);
            }
        }
    ...
    public function admin_page() {
        // Handle direct animal save
        if (isset($_POST['action']) && $_POST['action'] === 'save_animals_direct') {
            if (wp_verify_nonce($_POST['animals_nonce'], 'save_animals_direct')) {
                ...
                $result = $wpdb->update(
                    $wpdb->options,
                    array('option_value' => serialize($settings)),
                    array('option_name' => 'cc_chatbot_settings'),
                    array('%s'),
                    array('%s')
                );
                ...
                if ($result !== false) {
                    ...
                    $result2 = update_option('cc_chatbot_settings', $settings);
                    if ($result2) {
                        echo '<div class="notice notice-success is-dismissible"><p>Animal selection saved successfully!</p></div>';
                    } else {
                        echo '<div class="notice notice-error is-dismissible"><p>Error saving animal selection.</p></div>';
                    }
                }
            } else {
        ...
        <div class="wrap">
            ...
            <!-- Simple direct animal selection -->
            <div style="margin-top: 20px; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
                ...
                <p>This saves animal selection directly to the database.</p>
                ...
                // Add custom animals to available list (only enabled ones)
                $custom_animals = $settings['custom_animals'] ?? array();
                $enabled_custom_animals = array();
                foreach ($custom_animals as $custom_animal) {
                    if (isset($custom_animal['enabled']) && $custom_animal['enabled'] === '1') {
                        $available_animals[$custom_animal['id']] = $custom_animal['name'] . ' (Custom)';
                        $enabled_custom_animals[] = $custom_animal['id'];
                    }
                }
                
                // Build complete list of selected animals (avoiding duplication)
                $all_selected_animals = array_merge($selected_animals, $enabled_custom_animals);
                $all_selected_animals = array_unique($all_selected_animals);
                ?>
                
                <form method="post" action="">
                    <input type="hidden" name="action" value="save_animals_direct" />
                    <?php wp_nonce_field('save_animals_direct', 'animals_nonce'); ?>
                            ...
                            <td>
                                <?php foreach ($available_animals as $key => $label) : ?>
                                    <label style="display: block; margin-bottom: 5px;">
                                        <input type="checkbox" name="selected_animals[]" value="<?php echo esc_attr($key); ?>" 
                                               <?php checked(in_array($key, $all_selected_animals)); ?> />
                                        <?php echo esc_html($label); ?>
                                    </label>
                                <?php endforeach; ?>
