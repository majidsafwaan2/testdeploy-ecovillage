...
class CC_Admin {
    ...
    public function admin_page() {
        // Handle direct animal save with detailed debugging
        if (isset($_POST['action']) && $_POST['action'] === 'save_animals_direct') {
            echo '<div class="notice notice-info is-dismissible"><p>Form submitted - processing...</p></div>';
            
            if (wp_verify_nonce($_POST['animals_nonce'], 'save_animals_direct')) {
                echo '<div class="notice notice-info is-dismissible"><p>Nonce verified - getting settings...</p></div>';
                
                $settings = get_option('cc_chatbot_settings', array());
                echo '<div class="notice notice-info is-dismissible"><p>Current settings: ' . print_r($settings, true) . '</p></div>';
                ...
                if (isset($_POST['selected_animals']) && is_array($_POST['selected_animals'])) {
                    ...
                    echo '<div class="notice notice-info is-dismissible"><p>Selected animals: ' . print_r($selected_animals, true) . '</p></div>';
                } else {
                    echo '<div class="notice notice-info is-dismissible"><p>No animals selected in POST data</p></div>';
                }
                
                // Update settings
                $settings['selected_animals'] = $selected_animals;
                echo '<div class="notice notice-info is-dismissible"><p>Updated settings: ' . print_r($settings, true) . '</p></div>';
                ...
                if ($result !== false) {
                    echo '<div class="notice notice-success is-dismissible"><p>Animal selection saved successfully! (Direct DB write)</p></div>';
                } else {
                    // Try alternative method
                    $result2 = update_option('cc_chatbot_settings', $settings);
                    if ($result2) {
                        echo '<div class="notice notice-success is-dismissible"><p>Animal selection saved successfully! (update_option)</p></div>';
                    } else {
                        echo '<div class="notice notice-error is-dismissible"><p>Error saving animal selection. DB Error: ' . $wpdb->last_error . '</p></div>';
                    }
                }
            } else {
