 ...
 * @param {import('../types').ChatbotInstance} chatbotInstance - An instance of the chatbot logic,
 * providing the `respondTo` function and animal details.
 */
export function renderChatbotUI(containerElement, chatbotInstance) {
    // --- Input Validation ---
    ...
    // Ensure that a valid chatbot instance with a 'respondTo' method is provided.
    if (!chatbotInstance || typeof chatbotInstance.respondTo !== 'function') {
    ...
    launcherImg.src = chatbotInstance.getAnimalPhoto();
    launcherImg.alt = `${chatbotInstance.getAnimalName()} Avatar`;
    ...
    headerAvatar.src = chatbotInstance.getAnimalPhoto();
    headerAvatar.alt = `${chatbotInstance.getAnimalName()} Avatar`;
    headerAvatar.className = 'conservation-chatbot-avatar';
    const headerTitle = document.createElement('h3');
    headerTitle.textContent = `Talk to ${chatbotInstance.getAnimalName()}`;
    ...
    titleGroupDiv.appendChild(headerTitle);
        ...
        if (isChatOpen) {
            ...
            if (messagesContainer.children.length === 0 ||
                (messagesContainer.children.length === 1 && messagesContainer.children[0].classList.contains('thinking'))) {
                 addMessageToUI(`Hello! I'm ${chatbotInstance.getAnimalName()}. What would you like to know about me and my conservation?`, 'bot');
            }
    ...
    /**
     * Handles sending a message (either from user input or a default prompt)
     * to the chatbot instance and updating the UI accordingly.
     * It disables inputs, shows a thinking indicator, and handles AI response/errors.
     * @param {string} [userMessageInput] - Optional. The message text to send. If not provided,
     * the value from the `inputElement` will be used.
     */
    const sendMessage = async (userMessageInput) => {
        // Determine the message content: use the provided argument or the input field's value.
        const userMessage = userMessageInput || inputElement.value.trim();
        ...
        if (userMessage) {
            ...
            // Disable UI elements to prevent multiple submissions while waiting for AI
            sendButton.disabled = true;
            inputElement.disabled = true;
            defaultPromptsContainer.style.pointerEvents = 'none'; // Disable prompt buttons
            defaultPromptsContainer.style.opacity = '0.5';        // Visually indicate disabled state

            const thinkingIndicator = showThinkingIndicator(); // Show the thinking animation

            try {
                // Call the chatbot's AI response function
                const botResponse = await chatbotInstance.respondTo(userMessage);
                removeThinkingIndicator(thinkingIndicator); // Hide the thinking animation
                addMessageToUI(botResponse, 'bot');          // Display the AI's response
            } catch (error) {
                console.error("Error getting chatbot response:", error);
                removeThinkingIndicator(thinkingIndicator); // Hide thinking even if there's an error
                // Display a user-friendly error message in the chat
                addMessageToUI("Oops! I'm having trouble responding right now. Please try again later.", 'bot');
            } finally {
                // Re-enable UI elements regardless of success or failure
                sendButton.disabled = false;
                inputElement.disabled = false;
                defaultPromptsContainer.style.pointerEvents = 'auto'; // Re-enable prompt buttons
                defaultPromptsContainer.style.opacity = '1';
                inputElement.focus(); // Return focus to the input field
            }
        }
    };

    // --- Event Listeners for User Input and Prompts ---
    ...
    // Add event listeners for each default prompt button
    defaultPromptButtons.forEach(button => {
        button.addEventListener('click', () => {
            // Retrieve the predefined prompt text from the button's data attribute
            const promptText = button.dataset.prompt;
            sendMessage(promptText); // Send this prompt directly
        });
    });
}