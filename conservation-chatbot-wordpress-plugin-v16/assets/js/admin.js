...
jQuery(document).ready(function($) {
    ...
    $('#add-custom-animal').click(function() {
        console.log('Add custom animal button clicked');
        addCustomAnimal();
    });
    
    function addCustomAnimal() {
        console.log('addCustomAnimal function called');
    ...
    $('form').submit(function(e) {
        ...
        // Collect custom animals data before submission
        collectCustomAnimalsData();
    });
    
    // Function to collect custom animals data
    function collectCustomAnimalsData() {
        const customAnimals = {};
        
        // Find all custom animal containers
        $('.custom-animal').each(function() {
            const animalId = $(this).data('animal-id');
            const animalData = {};
            
            // Collect all form fields for this animal
            $(this).find('input, select, textarea').each(function() {
                const fieldName = $(this).attr('name');
                if (fieldName && fieldName.includes('cc_custom_animals[')) {
                    // Extract the field name from the array notation
                    const match = fieldName.match(/cc_custom_animals\[([^\]]+)\]\[([^\]]+)\]/);
                    if (match) {
                        const fieldKey = match[2];
                        animalData[fieldKey] = $(this).val();
                    }
                }
            });
            
            // Only add if animal has at least a name
            if (animalData.name && animalData.name.trim() !== '') {
                customAnimals[animalId] = animalData;
            }
        });
        
        // Update the hidden field with collected data
        $('input[name="cc_custom_animals"]').val(JSON.stringify(customAnimals));
    }
    ...
    // Initialize API key field state and load existing custom animals
    $(document).ready(function() {
        ...
        // Load existing custom animals
        loadExistingCustomAnimals();
    });
    
    // Function to load existing custom animals
    function loadExistingCustomAnimals() {
        const customAnimalsField = $('input[name="cc_custom_animals"]');
        ...
        if (customAnimalsData) {
            try {
                ...
                Object.keys(customAnimals).forEach(function(animalId) {
                    const animalData = customAnimals[animalId];
                    
                    // Create the animal container
                    const container = $('#custom-animals-container');
                    const animalHtml = `
                        <div class="custom-animal" data-animal-id="${animalId}">
                            <h4>Custom Animal</h4>
                            <table class="form-table">
                                <tr>
                                    <th scope="row">Animal ID</th>
                                    <td>
                                        <input type="text" name="cc_custom_animals[${animalId}][id]" value="${animalData.id || animalId}" class="regular-text" required />
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">Name</th>
                                    <td>
                                        <input type="text" name="cc_custom_animals[${animalId}][name]" value="${animalData.name || ''}" class="regular-text" placeholder="Animal Name" required />
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">Species</th>
                                    <td>
                                        <input type="text" name="cc_custom_animals[${animalId}][species]" value="${animalData.species || ''}" class="regular-text" placeholder="Species" required />
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">Conservation Status</th>
                                    <td>
                                        <select name="cc_custom_animals[${animalId}][conservationStatus]">
                                            <option value="Endangered" ${animalData.conservationStatus === 'Endangered' ? 'selected' : ''}>Endangered</option>
                                            <option value="Vulnerable" ${animalData.conservationStatus === 'Vulnerable' ? 'selected' : ''}>Vulnerable</option>
                                            <option value="Near Threatened" ${animalData.conservationStatus === 'Near Threatened' ? 'selected' : ''}>Near Threatened</option>
                                            <option value="Least Concern" ${animalData.conservationStatus === 'Least Concern' ? 'selected' : ''}>Least Concern</option>
                                            <option value="Critically Endangered" ${animalData.conservationStatus === 'Critically Endangered' ? 'selected' : ''}>Critically Endangered</option>
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">Location</th>
                                    <td>
                                        <input type="text" name="cc_custom_animals[${animalId}][location]" value="${animalData.location || ''}" class="regular-text" placeholder="Location" required />
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">Photo URL</th>
                                    <td>
                                        <input type="url" name="cc_custom_animals[${animalId}][photo]" value="${animalData.photo || ''}" class="regular-text" placeholder="https://example.com/image.jpg" required />
                                        <button type="button" class="button upload-image-btn" data-target="${animalId}">Upload Image</button>
                                    </td>
                                </tr>
                                <tr>
                                    <th scope="row">Introduction</th>
                                    <td>
                                        <textarea name="cc_custom_animals[${animalId}][intro]" class="large-text" rows="3" placeholder="Introduction message for this animal">${animalData.intro || ''}</textarea>
                                    </td>
                                </tr>
                            </table>
                            <button type="button" class="button remove-custom-animal" data-animal-id="${animalId}">Remove Animal</button>
                ...
                console.error('Error loading custom animals:', error);
            }
        }
    }
            apiKeyField.attr('type', 'password');
