...
class CC_Admin {
    ...
    public function init() {
        ...
        // Add AJAX handler for animal selection
        add_action('wp_ajax_cc_save_animal_selection', array($this, 'ajax_save_animal_selection'));
        add_action('wp_ajax_nopriv_cc_save_animal_selection', array($this, 'ajax_save_animal_selection'));
        
        // Add test AJAX handler
        add_action('wp_ajax_cc_test_ajax', array($this, 'test_ajax'));
        add_action('wp_ajax_nopriv_cc_test_ajax', array($this, 'test_ajax'));
        
        error_log('CC Animal Selection - AJAX handlers registered');
    }
    ...
    public function enqueue_admin_scripts($hook) {
        ...
        wp_localize_script('cc-admin', 'cc_ajax', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('cc_animal_selection'),
            'debug' => true
        ));
    }
    ...
    public function sanitize_settings($input) {
        ...
        // Animal settings - FORCE PROCESSING
        ...
        if (isset($input['force_process_animals'])) {
            ...
            error_log('CC Debug - Form not submitted, keeping existing animals');
        }
    ...
    /**
     * AJAX handler for animal selection
     */
    public function ajax_save_animal_selection() {
        try {
            // Simple test to see if handler is called
            error_log('CC Animal Selection - AJAX handler called at: ' . date('Y-m-d H:i:s'));
            error_log('CC Animal Selection - Handler file: ' . __FILE__);
            error_log('CC Animal Selection - Handler line: ' . __LINE__);
            error_log('CC Animal Selection - POST data: ' . print_r($_POST, true));
            error_log('CC Animal Selection - REQUEST_METHOD: ' . $_SERVER['REQUEST_METHOD']);
            error_log('CC Animal Selection - HTTP_REFERER: ' . ($_SERVER['HTTP_REFERER'] ?? 'none'));
            
            // Verify nonce
            if (!isset($_POST['nonce'])) {
                error_log('CC Animal Selection - No nonce provided');
                wp_send_json_error(array('message' => 'No nonce provided'));
                return;
            }
            
            if (!wp_verify_nonce($_POST['nonce'], 'cc_animal_selection')) {
                error_log('CC Animal Selection - Nonce verification failed');
                wp_send_json_error(array('message' => 'Security check failed'));
                return;
            }
            
            error_log('CC Animal Selection - Nonce verification passed');
            
            // Get current settings
            $settings = get_option('cc_chatbot_settings', array());
            error_log('CC Animal Selection - Current settings: ' . print_r($settings, true));
            ...
            if (isset($_POST['selected_animals']) && is_array($_POST['selected_animals'])) {
                ...
                error_log('CC Animal Selection - Processed selected animals: ' . print_r($selected_animals, true));
            } else {
                error_log('CC Animal Selection - No selected animals in POST data');
            }
            
            // Update settings
            $settings['selected_animals'] = $selected_animals;
            error_log('CC Animal Selection - Updated settings: ' . print_r($settings, true));
            ...
            if ($result) {
                error_log('CC Animal Selection - Sending success response');
                wp_send_json_success(array(
                    'message' => 'Animal selection saved successfully!',
                    'selected_animals' => $selected_animals
                ));
            } else {
                error_log('CC Animal Selection - Sending error response');
                wp_send_json_error(array(
                    'message' => 'Error saving animal selection. Please try again.'
                ));
            }
        } catch (Exception $e) {
            error_log('CC Animal Selection - Exception caught: ' . $e->getMessage());
            wp_send_json_error(array(
    ...
    public function test_ajax() {
        error_log('CC Test AJAX - Handler called');
    ...
    public function admin_page() {
        ...
        <div class="wrap">
            ...
            <?php
            // Show success/error messages for animal selection
            if (isset($_GET['animal_saved']) && $_GET['animal_saved'] == '1') {
                echo '<div class="notice notice-success is-dismissible"><p>Animal selection saved successfully!</p></div>';
            }
            if (isset($_GET['animal_error']) && $_GET['animal_error'] == '1') {
                echo '<div class="notice notice-error is-dismissible"><p>Error saving animal selection. Please try again.</p></div>';
            }
            ?>
            ...
            <form method="post" action="options.php">
                ...
                <?php wp_nonce_field('cc_admin_nonce', 'cc_admin_nonce'); ?>
            ...
            <!-- AJAX-based animal selection -->
            <div style="margin-top: 20px; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
                <h3>Animal Selection (AJAX Save)</h3>
                <p>This saves animal selection using AJAX, bypassing WordPress settings API completely.</p>
                ...
                <table class="form-table">
                    <tr>
                        <th scope="row">Selected Animals</th>
                        <td>
                            <?php foreach ($available_animals as $key => $label) : ?>
                                <label style="display: block; margin-bottom: 5px;">
                                    <input type="checkbox" class="animal-checkbox" value="<?php echo esc_attr($key); ?>" 
                                           <?php checked(in_array($key, $selected_animals)); ?> />
                                    <?php echo esc_html($label); ?>
                                </label>
                            <?php endforeach; ?>
                ...
                <button type="button" id="save-animal-selection" class="button button-primary">Save Animal Selection</button>
                <button type="button" id="test-ajax" class="button button-secondary" style="margin-left: 10px;">Test AJAX</button>
                <span id="animal-save-status" style="margin-left: 10px;"></span>
